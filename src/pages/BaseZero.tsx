...
function ModuleChallenge({ moduleId, lang, completed, onComplete, setFeedback }: { moduleId: BaseZeroModuleId; lang: 'en' | 'pt'; completed: boolean; onComplete: () => void; setFeedback: (message: string) => void }) {
  ...
  if (moduleId === 'files') return (
    <div className="hp-practice-card rounded-xl p-4 space-y-3">
      ...
      <input value={folder} onChange={event => setFolder(event.target.value)} aria-label={lang === 'en' ? 'Folder name' : 'Nome da pasta'} placeholder={lang === 'en' ? 'Folder name' : 'Nome da pasta'} className="w-full rounded-lg px-3 py-2.5 text-sm" style={{ background: 'var(--c-bg)', color: 'var(--c-text)', border: '1px solid var(--c-border)' }} />
      <input value={file} onChange={event => setFile(event.target.value)} aria-label={lang === 'en' ? 'File name with extension' : 'Nome do arquivo com extensão'} placeholder={lang === 'en' ? 'File name with extension' : 'Nome do arquivo com extensão'} className="w-full rounded-lg px-3 py-2.5 text-sm" style={{ background: 'var(--c-bg)', color: 'var(--c-text)', border: '1px solid var(--c-border)' }} />
