...
print(f"Critical: {critical}")`,
    ...
    },
    {
      ...
      description: {
        en: 'Build this program from scratch. Every line goes into the blue editor.\n\nThe complete analysis is already provided in the starter — read each line carefully, then run it as-is.\n\nWhat each statistic means:\ntotal = sum(sales) — adds all daily totals\naverage = total / len(sales) — divides total by number of days\nminimum = min(sales) and maximum = max(sales) — smallest and largest values\nmedian = sorted(sales)[len(sales)//2] — the middle value after sorting\nnet_total = sum(c - 250 for c in sales) — total net_total after $250 discount per sale\ncritical = len([c for c in sales if c > 8000]) — count of sales above $8000\nurgent = len([c for c in sales if 3000 <= c <= 8000]) — count in the $3000–$8000 range\nnormal = len([c for c in sales if c < 3000]) — count below $3000\ntop3 = sorted(sales, reverse=True)[:3] — the 3 largest values\n\nExpected output:\n=== REPORT ===\nTotal: $42,400',
        pt: 'Construa este programa do zero. Cada linha vai no editor azul.\n\nA análise completa já está fornecida no starter — leia cada linha com atenção e execute como está.\n\nO que cada estatística significa:\ntotal = sum(sales) — soma todos os valores de dano\naverage = total / len(sales) — divide o total pelo número de dias\nminimum = min(sales) e maximum = max(sales) — menor e maior valores\nmedian = sorted(sales)[len(sales)//2] — o valor do meio após ordenação\nnet_total = sum(c - 250 for c in sales) — net_total total após crédito de $250 por venda\ncritical = len([c for c in sales if c > 8000]) — contagem de vendas acima de $8000\nurgent = len([c for c in sales if 3000 <= c <= 8000]) — contagem na faixa $3000–$8000\nnormal = len([c for c in sales if c < 3000]) — contagem abaixo de $3000\ntop3 = sorted(sales, reverse=True)[:3] — os 3 maiores valores\n\nSaída esperada:\n=== REPORT ===\nTotal: $42,400'
      },
