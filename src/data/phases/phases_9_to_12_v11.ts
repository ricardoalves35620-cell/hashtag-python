...
for material, esperado, recebido in entregas:
  ...
  exercises: [
    ...
    exercise(
      ...
      b('Goal:\nWrite the function approved_total(rows, platform_fee). Each row is a list in the shape [name, amount, status]. Sum the total for every approved row, where the total for each row is the amount minus the platform fee, never dropping below zero.\n\nProgram requirements\n\n1. Calculate\n- For each row, unpack the name, amount and status\n- For approved rows only, compute the total as the amount minus the platform fee, using zero as a floor\n- Add up all totals and return the total\n\nExample, with approved sales and a platform fee of 300:\n7700', 'Objetivo:\nEscreva a função approved_total(rows, platform_fee). Cada linha é uma lista no formato [nome, valor, status]. Some o pagamento de cada linha aprovada, onde o pagamento é o valor menos a taxa da plataforma, nunca ficando abaixo de zero.\n\nRequisitos do programa\n\n1. Calcular\n- Para cada linha, desempacote nome, valor e status\n- Somente para linhas aprovadas, calcule o pagamento como o valor menos a taxa da plataforma, usando zero como piso\n- Some todos os pagamentos e retorne o total\n\nExemplo, com vendas aprovadas e taxa de plataforma de 300:\n7700'),
...
print(atualizado["confirmado"])   # True`,
  ...
  exercises: [
    exercise(
      ...
      b('Goal:\nComplete product_label so it returns a text in the format "sku - name" by reading two keys from the product dictionary.\n\nProgram requirements\n\n- Blank 1 — the key for the product code\n- Blank 2 — the key for the product name\n\nExample, for a product with sku "KB-10" and name "Keyboard":\nKB-10 - Keyboard', 'Objetivo:\nComplete product_label para que ela retorne um texto no formato "sku - name" lendo duas chaves do dicionário product.\n\nRequisitos do programa\n\n- Lacuna 1 — a chave do código do produto\n- Lacuna 2 — a chave do nome do produto\n\nExemplo, para um produto com sku "KB-10" e name "Keyboard":\nKB-10 - Keyboard'),
    ...
    exercise(
      ...
      b('Goal:\nFinish sell_units so it returns an updated copy of the product with the stock reduced, following these rules:\n\nProgram requirements\n\n1. Validate\n- Raise ValueError("negative sale") when the sold amount is negative\n\n2. Calculate\n- Create a copy of the original product so it is never modified\n- Reduce the stock by the sold amount, but never let it drop below zero\n- Return the updated copy\n\n- Blank — create a copy of the product dictionary\n\nExample, selling 3 units from a product with stock 5:\n{\'sku\': \'A1\', \'stock\': 2}', 'Objetivo:\nTermine sell_units para que ela retorne uma cópia atualizada do produto com estoque reduzido, seguindo estas regras:\n\nRequisitos do programa\n\n1. Validar\n- Gere ValueError("negative sale") quando a quantidade vendida for negativa\n\n2. Calcular\n- Crie uma cópia do produto original para que ele nunca seja modificado\n- Reduza o estoque pela quantidade vendida, sem nunca deixar abaixo de zero\n- Retorne a cópia atualizada\n\n- Lacuna — crie uma cópia do dicionário do produto\n\nExemplo, vendendo 3 unidades de um produto com estoque 5:\n{\'sku\': \'A1\', \'stock\': 2}'),
...
print(item)`, b("{'sku': 'B2', 'stock': 0}\n{'sku': 'B2', 'stock': 2}", "{'sku': 'B2', 'stock': 0}\n{'sku': 'B2', 'stock': 2}"), 40, [{ kind: 'function', value: 'sell_units' }, { kind: 'call', value: 'copy' }], true),
    ...
    exercise(
      ...
      b('Goal:\nWrite calculate_total(order). The order is a dictionary with required keys quantity and unit_price, and an optional key discount that defaults to zero.\n\nProgram requirements\n\n1. Validate\n- Raise ValueError("missing keys") when a required key is absent\n- Raise ValueError("negative value") when quantity, unit_price or discount is negative\n\n2. Calculate\n- Multiply quantity by unit_price, applying the discount if present\n- Round the result to two decimal places and return the total\n\nExample, for an order with quantity 10, unit_price 5 and discount 0.1:\n45.0', 'Objetivo:\nEscreva calculate_total(order). O order é um dicionário com as chaves obrigatórias quantity e unit_price, e uma chave opcional discount que vale zero por padrão.\n\nRequisitos do programa\n\n1. Validar\n- Gere ValueError("missing keys") quando faltar uma chave obrigatória\n- Gere ValueError("negative value") quando quantity, unit_price ou discount for negativo\n\n2. Calcular\n- Multiplique quantity por unit_price, aplicando o desconto se presente\n- Arredonde o resultado para duas casas decimais e retorne o total\n\nExemplo, para um pedido com quantity 10, unit_price 5 e discount 0.1:\n45.0'),
...
print(fila)`,
  ...
  exercises: [
    exercise(
      ...
      b('Goal:\nComplete active_names so it returns a list with the name of every record whose active field is True.\n\nProgram requirements\n\n- Blank 1 — the key that indicates whether the record is active\n- Blank 2 — the key that holds the record\'s name\n\nExample, for records where Ana and João are active:\n[\'Ana\', \'João\']', 'Objetivo:\nComplete active_names para que ela retorne uma lista com o nome de todo registro cujo campo active seja True.\n\nRequisitos do programa\n\n- Lacuna 1 — a chave que indica se o registro está ativo\n- Lacuna 2 — a chave que guarda o nome do registro\n\nExemplo, para registros onde Ana e João estão ativos:\n[\'Ana\', \'João\']'),
    ...
    exercise(
      ...
      b('Goal:\nFinish inventory_value so it returns the total inventory value across all products. Each product contributes its price multiplied by its stock.\n\nProgram requirements\n\n1. Validate\n- Raise ValueError("negative product") when a product has a negative price or stock\n\n2. Calculate\n- For each product, multiply price by stock and add it to the total\n- Return the accumulated total\n\n- Blank 1 — raise the error for invalid numeric data\n- Blank 2 — the contribution of one product to the total\n\nExample, with products priced at 10 (stock 5) and 15 (stock 2):\n80', 'Objetivo:\nTermine inventory_value para que ela retorne o valor total do estoque de todos os produtos. Cada produto contribui com seu preço multiplicado pelo estoque.\n\nRequisitos do programa\n\n1. Validar\n- Gere ValueError("negative product") quando um produto tiver preço ou estoque negativo\n\n2. Calcular\n- Para cada produto, multiplique preço por estoque e adicione ao total\n- Retorne o total acumulado\n\n- Lacuna 1 — gere o erro para dados numéricos inválidos\n- Lacuna 2 — a contribuição de um produto ao total\n\nExemplo, com produtos com preço 10 (estoque 5) e preço 15 (estoque 2):\n80'),
    ...
    exercise(
      ...
      b('Goal:\nWrite index_by_id(records). Build and return a dictionary that maps each record\'s id to an independent copy of that record.\n\nProgram requirements\n\n1. Validate\n- Raise ValueError("duplicate id") when two records share the same id\n\n2. Calculate\n- For each record, store an independent copy keyed by its id\n- Return the resulting index dictionary\n\nExample, for records with ids 1 and 2:\n{1: {\'id\': 1, \'name\': \'Ana\'}, 2: {\'id\': 2, \'name\': \'Beto\'}}', 'Objetivo:\nEscreva index_by_id(records). Construa e retorne um dicionário que mapeie o id de cada registro para uma cópia independente desse registro.\n\nRequisitos do programa\n\n1. Validar\n- Gere ValueError("duplicate id") quando dois registros compartilharem o mesmo id\n\n2. Calcular\n- Para cada registro, armazene uma cópia independente usando seu id como chave\n- Retorne o dicionário índice resultante\n\nExemplo, para registros com ids 1 e 2:\n{1: {\'id\': 1, \'name\': \'Ana\'}, 2: {\'id\': 2, \'name\': \'Beto\'}}'),
...
print(horas_longas)  # [1.25, 2.0]`,
  ...
  exercises: [
    exercise(
      ...
      b('Goal:\nComplete even_squares with one list comprehension that returns the square of every even number in the given list.\n\nProgram requirements\n\n- Blank 1 — the transformation applied to each qualifying number\n- Blank 2 — the condition that keeps only even numbers\n\nExample, for the list [1, 2, 3, 4, 6]:\n[4, 16, 36]', 'Objetivo:\nComplete even_squares com uma compreensão de lista que retorne o quadrado de todo número par na lista recebida.\n\nRequisitos do programa\n\n- Lacuna 1 — a transformação aplicada a cada número selecionado\n- Lacuna 2 — a condição que mantém apenas números pares\n\nExemplo, para a lista [1, 2, 3, 4, 6]:\n[4, 16, 36]'),
    ...
    exercise(
      ...
      b('Goal:\nFinish clean_names using a list comprehension that normalizes each name while discarding blank ones.\n\nProgram requirements\n\n1. Calculate\n- Strip surrounding whitespace from each name, then convert to lowercase\n- Remove names that are empty or contain only spaces\n- Preserve accented characters as they are\n\n- Blank 1 — strip surrounding spaces\n- Blank 2 — convert to lowercase\n- Blank 3 (filter) — keep only non-empty names after stripping\n\nExample, for names [" ana ", "joão", "  "]:\n[\'ana\', \'joão\']', 'Objetivo:\nTermine clean_names usando uma compreensão de lista que normaliza cada nome descartando os vazios.\n\nRequisitos do programa\n\n1. Calcular\n- Remova espaços externos de cada nome e depois converta para minúsculas\n- Descarte nomes que estejam vazios ou contenham apenas espaços\n- Preserve caracteres acentuados como são\n\n- Lacuna 1 — remova espaços externos\n- Lacuna 2 — converta para minúsculas\n- Lacuna 3 (filtro) — mantenha apenas nomes não vazios após remover espaços\n\nExemplo, para os nomes [" ana ", "joão", "  "]:\n[\'ana\', \'joão\']'),
    ...
    exercise(
      ...
      b('Goal:\nWrite available_labels(products). Use one list comprehension to return labels in the format "sku:name" for every product with stock greater than zero.\n\nProgram requirements\n\n1. Validate\n- Raise ValueError("negative stock") if any product has a negative stock value\n\n2. Calculate\n- Use a comprehension to filter products with stock greater than zero\n- For each qualifying product, produce a label combining its sku and name keys\n- Return the list of labels\n\nExample, for products with skus A (Mouse), B (out of stock) and C (Café):\n[\'A:Mouse\', \'C:Café\']', 'Objetivo:\nEscreva available_labels(products). Use uma compreensão de lista para retornar rótulos no formato "sku:name" para cada produto com estoque maior que zero.\n\nRequisitos do programa\n\n1. Validar\n- Gere ValueError("negative stock") se algum produto tiver estoque negativo\n\n2. Calcular\n- Use uma compreensão para filtrar produtos com estoque maior que zero\n- Para cada produto selecionado, produza um rótulo combinando suas chaves sku e name\n- Retorne a lista de rótulos\n\nExemplo, para produtos com skus A (Mouse), B (sem estoque) e C (Café):\n[\'A:Mouse\', \'C:Café\']'),
