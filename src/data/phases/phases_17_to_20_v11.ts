...
export const phase17: Phase = {
  ...
  exercises: [
    ...
    makeExercise({
      ...
      starterCode: 'def parse_stock(text):  # turn raw text into a list of stock records\n    records = []  # collect the finished records here\n    for raw_line in text.splitlines():  # go through the text one line at a time\n        if not raw_line.strip():  # skip blank lines\n            continue\n        sku, name, quantity_text = [part.strip() for part in raw_line.split("|")]  # split on | and trim each piece\n        quantity = int(quantity_text)  # turn the quantity text into a number\n        # Validate and append the normalized record.\n    return records  # hand back everything collected',
...
export const phase18: Phase = {
  ...
  exercises: [
    ...
    makeExercise({
      ...
      starterCode: 'def save_lines(path, lines):  # write each line to a file and count them\n    count = 0  # nothing written yet\n    with open(path, "w", encoding="utf-8") as file:  # open for writing, auto-close after\n        for item in lines:  # go through each line to write\n            # Write exactly one line and increment count.\n            pass\n    return count  # hand back how many were written', sampleOutput: b('3\nalpha\nação\n99', '3\nalpha\nação\n99'),
