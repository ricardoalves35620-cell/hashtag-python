...
describe('Learning Engine V2.11 foundation hardening', () => {
  ...
  it('gives the Phase 12 mini-project separate normal and edge evidence', () => {
    ...
    expect(project?.starterCode.en).toContain('input("Order: ")')
    expect(project?.starterCode.pt).toContain('input("Pedido: ")')
    expect(project?.tests.some(test => test.id === 'portfolio-accents-decimals')).toBe(true)
  })
})
