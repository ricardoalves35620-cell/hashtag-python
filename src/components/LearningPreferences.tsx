...
export default function LearningPreferences({ lang }: { lang: Lang }) {
  ...
  return <section className="rounded-2xl p-4 space-y-3" style={{ background: 'var(--c-card)', border: '1px solid var(--c-border)' }}>
    ...
    {/* The checkbox was a bare <input type="checkbox">, which the browser renders at
        13x13 CSS px. That fails WCAG 2.2 SC 2.5.8 (Target Size Minimum, 24x24) and is
        genuinely hard to hit on a phone — these are settings a learner uses. The row
        is already a <label>, so making it a comfortable target costs nothing. */}
    {keys.map(key => <label key={key} className="flex cursor-pointer items-center justify-between gap-4 rounded-xl p-3" style={{ background: 'var(--c-bg)', border: '1px solid var(--c-border)', minHeight: 48 }}>
