...
export default function ExamExpectedOutput({ exam, lang, compact = false }: { exam: Exam; lang: Lang; compact?: boolean }) {
  ...
  const copy = lang === 'pt' ? {
    ...
    exact: 'É exatamente isto que esperamos ver na tela. Neste exame, pequenas variações só são aceitas quando o enunciado permitir.',
    ...
    exact: 'This is exactly what we expect to see on screen. In an exam, small variations are accepted only when the question allows them.',
      ...
      <div style={{ display: 'grid', gap: 10 }}>
        {contracts.map((contract, index) => (
          <article key={contract.id} style={{ background: 'var(--c-bg)', border: '1px solid var(--c-border)', borderRadius: 10, padding: 12 }}>
            {contracts.length > 1 && <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--c-text)', marginBottom: 8 }}>{index + 1}. {personalize(contract.description)}</div>}
            ...
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--c-text2)', lineHeight: 1.65, marginBottom: 10 }}>
              {contract.inputs.length === 0 && contract.inputMap.length === 0
                ...
                : <>
                    {contract.inputs.map((value, inputIndex) => <div key={`input-${inputIndex}`}>{inputIndex + 1}. {personalize(value)}</div>)}
                    {contract.inputMap.map(item => <div key={item.label}>{item.label}: {personalize(item.value)}</div>)}
                  </>}
            </div>
            <pre style={{ margin: 0, whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', fontFamily: 'var(--font-mono)', fontSize: compact ? 12 : 13, lineHeight: 1.7, color: 'var(--c-success-text)' }}>{personalize(contract.expected)}</pre>
